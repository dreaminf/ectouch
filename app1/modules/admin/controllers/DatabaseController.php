...
class DatabaseController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'backup') {
            ...
            /* 权限检查 */
            $path = public_path(DATA_DIR . '/sqldata/');
            ...
            $this->smarty->assign('sql_name', SqlDump::get_random_name() . '.sql');
        ...
        if ($_REQUEST['act'] == 'dumpsql') {
            ...
            /* 初始化 */
            $dump = new SqlDump($this->db);
            $run_log = public_path(DATA_DIR . '/sqldata/run.log');
            ...
            switch ($type) {
                case 'full':
                    $except = [$this->ecs->prefix . 'sessions', $this->ecs->prefix . 'sessions_data'];
            ...
            if (empty($tables)) {
                ...
                if ($vol > 1) {
                    /* 有多个文件 */
                    if (!@file_put_contents(public_path(DATA_DIR . '/sqldata/' . $sql_file_name . '_' . $vol . '.sql'), $dump->dump_sql)) {
                    ...
                    /* 只有一个文件 */
                    if (!@file_put_contents(public_path(DATA_DIR . '/sqldata/' . $sql_file_name . '.sql'), $dump->dump_sql)) {
                        return sys_msg(sprintf($GLOBALS['_LANG']['fail_write_file'], $sql_file_name . '_' . $vol . '.sql'), 1, [['text' => $GLOBALS['_LANG']['02_db_manage'], 'href' => 'database.php?act=backup']], false);
                    };
        ...
        if ($_REQUEST['act'] == 'upload_sql') {
            ...
            $sql_file = public_path(DATA_DIR . '/upload_database_bak.sql');
