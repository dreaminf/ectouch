    ...
    if ($type == 0)
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            if(preg_match("/^http/", $row['original_img']) && @fopen($row['original_img'], 'r')){
                if(preg_match('/(.jpg|.png|.gif|.jpeg)$/',$row['original_img']) && @copy(trim($row['original_img']), ROOT_PATH . 'data/attachment/' . basename($row['original_img'])))
                {
                    $is_remote_img = true;
                    $row['original_img'] = $row['goods_img'] = 'data/attachment/' . basename($row['original_img']);
                }
            }
