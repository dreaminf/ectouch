...
if ($_REQUEST['act'] == 'remove_msg')
{
    ...
    if ($row)
    {
        if ($row['user_id'] == $user_id && $row['order_id'] == $order_id)
        {
            if ($row['message_img'])
            {
                @unlink(ROOT_PATH. DATA_DIR . '/attachment/feedbackimg/' . $row['message_img']);
            }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($msg_id))
    {
        ...
        if (!empty($img))
        {
             @unlink(ROOT_PATH. DATA_DIR . '/attachment/feedbackimg/'.$img);
        }
...
elseif ($_REQUEST['act'] == 'drop_file')
{
    ...
    @unlink('../' . DATA_DIR . '/attachment/feedbackimg/'.$file);
