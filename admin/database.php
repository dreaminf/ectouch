...
if ($_REQUEST['act'] == 'backup')
{
    ...
    /* 权限检查 */
    $path = ROOT_PATH . DATA_DIR . '/attachment/sqldata';
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    $run_log = ROOT_PATH . DATA_DIR . '/attachment/sqldata/run.log';
    ...
    if (empty($tables))
    {
        ...
        {
            /* 有多个文件 */
            if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/attachment/sqldata/' . $sql_file_name . '_' . $vol . '.sql', $dump->dump_sql))
            {
            ...
            for ($i = 1; $i <= $vol; $i++)
            {
                $list[] = array('name'=>$sql_file_name . '_' . $i . '.sql', 'href'=>'../' . DATA_DIR . '/attachment/sqldata/' . $sql_file_name . '_' . $i . '.sql');
            }
        ...
        {
            /* 只有一个文件 */
            if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/attachment/sqldata/' . $sql_file_name . '.sql', $dump->dump_sql))
            {
                sys_msg(sprintf($_LANG['fail_write_file'], $sql_file_name . '_' . $vol . '.sql'), 1, array(array('text'=>$_LANG['02_db_manage'], 'href'=>'database.php?act=backup')), false);
            };

            $smarty->assign('list',  array(array('name' => $sql_file_name . '.sql', 'href' => '../' . DATA_DIR . '/attachment/sqldata/' . $sql_file_name . '.sql')));
            $smarty->assign('title', $_LANG['backup_success']);
            $smarty->display('sql_dump_msg.htm');
        }
    }
...
if ($_REQUEST['act'] == 'upload_sql')
{
    ...
    $sql_file = ROOT_PATH . DATA_DIR . '/attachment/upload_database_bak.sql';
