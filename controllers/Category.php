...
class Category extends IndexController {
    ...
    public function index() {
        ...
        // 获取分类
        $this->assign('category', get_top_category());
        $count = category_get_count($this->children, $this->brand, $this->type, $this->price_min, $this->price_max, $this->ext,$this->keywords);
        ...
        $cat = get_cat_info($this->cat_id);  // 获得分类的相关信息
        ...
        $this->assign('categories', get_categories_tree($this->cat_id));
    ...
    private function parameter() {
        ...
        /* 赋值固定内容 */
        if ($this->brand > 0) {
            $brand_name = $this->model->table('brand')->field('brand_name')->where("brand_id = '$this->brand'")->getOne();
        } else {
            $brand_name = '';
        }
        ...
        if ($cat['grade'] == 0 && $cat['parent_id'] != 0) {
            $cat['grade'] = get_parent_grade($this->cat_id); // 如果当前分类级别为空，取最近的上级分类
        }
        ...
        if ($cat['grade'] > 1) {
            ...
            $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max " . " FROM " . $this->model->pre . 'goods' . " AS g " . " WHERE ($this->children OR " . get_extension_goods($this->children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  ';
            // 获得当前分类下商品价格的最大值、最小值
            ...
            $sql = "SELECT (FLOOR((g.shop_price - $row[min]) / $dx)) AS sn, COUNT(*) AS goods_num  " . " FROM " . $this->model->pre . 'goods' . " AS g " . " WHERE ($this->children OR " . get_extension_goods($this->children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . " GROUP BY sn ";
        ...
        if ($cat['filter_attr'] > 0) {
            ...
            foreach ($cat_filter_attr as $key => $value) {
                $sql = "SELECT a.attr_name FROM " . $this->model->pre . "attribute AS a, " . $this->model->pre . "goods_attr AS ga, " . $this->model->pre . "goods AS g WHERE ($this->children OR " . get_extension_goods($this->children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
                $res = $this->model->query($sql);
                if ($temp_name = $res[0]['attr_name']) {
                    $all_attr_list[$key]['filter_attr_id'] = $value; // 新增属性标识 by wang
                    $all_attr_list[$key]['filter_attr_name'] = $temp_name;

                    $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $this->model->pre . "goods_attr AS a, " . $this->model->pre . "goods AS g" . " WHERE ($this->children OR " . get_extension_goods($this->children) . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . " AND a.attr_id='$value' " . " GROUP BY a.attr_value";
    ...
    public function top_all() {
        ...
        if (!$this->load->tpl->is_cached('category_all.dwt', $cache_id)) {
    ...
    private function category_get_goods() {
        ...
        if ($this->keywords != '') {
            ...
            $where.=" AND ($this->children OR " . get_extension_goods($this->children) . ') ';
        }
        if ($this->type) {
            switch ($this->type) {
                case 'best':
        ...
        foreach ($res as $row) {
            ...
            if (mt_rand(0, 3) == 3){
                $sales_volume = get_sales_count($row['goods_id']);
            ...
            $arr[$row['goods_id']]['comment_count'] = get_goods_comment($row['goods_id'], 0);  //商品总评论数量 
            $arr[$row['goods_id']]['favorable_count'] = favorable_comment($row['goods_id'], 0);  //获得商品好评百分比
        }
        return $arr;
    }
