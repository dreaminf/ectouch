...
class Flow extends IndexController {
    ...
    public function login() {
        ...
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            if ($act == 'signin') {
                ...
                if (register(trim($username), trim($password), trim($email))) {
                    /* 用户注册成功 */
    ...
    public function consignee() {
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            if ($_SESSION ['user_id'] > 0) {
                $addressId = I('get.id');
                if ($addressId > 0) {
                    $consignee_list[] = get_consignee_list($_SESSION ['user_id'], $addressId);
                } else {
    ...
    public function done() {
        ...
        /* 检查商品总额是否达到最低限购金额 */
        if ($flow_type == CART_GENERAL_GOODS && cart_amount(true, CART_GENERAL_GOODS) < C('min_goods_amount')) {
        ...
        do {
            ...
            $new_order = filter_field('order_info', $order);
        ...
        // 销量
        add_touch_goods($flow_type, $order ['extension_code']);
        /* 清空购物车 */
            ...
            if ($order_sn) {
                ...
                if ($payment_info ['pay_code'] == 'balance' && $order_info ['order_amount'] > 0) {
                    ...
                    if ($order_info ['user_id'] > 0 && $order ['surplus'] > 0) {
                        log_account_change($order_info ['user_id'], $order ['surplus'] * (- 1), 0, 0, 0, sprintf(L('pay_order'), $order_info ['order_sn']));
                    }
    ...
    public function consignee_list() {
        if (IS_AJAX) {
            ...
            $limit = $_POST ['amount'];
            // 获得用户所有的收货人信息
            $consignee_list = get_consignee_list($_SESSION['user_id'], 0, $limit, $start);
            if ($consignee_list) {
