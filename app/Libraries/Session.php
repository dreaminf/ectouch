...
class Session
{
    ...
    public function __construct(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        if ($session_id == '' && cookie('?' . $this->session_name)) {
        ...
        if ($this->session_id) {
            ...
            cookie($this->session_name, $this->session_id . $this->gen_session_key($this->session_id));
        }
    ...
    public function gen_session_key($session_id)
    {
        ...
        return sprintf('%08x', crc32(base_path() . $ip . $session_id));
    }
