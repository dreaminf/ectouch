...
//             }
    ...
    public function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
    {
        ...
        /* 检查email是否重复 */
        //$sql = "SELECT " . $this->field_id . " FROM " . $this->db->table($this->user_table) . " WHERE " . $this->field_email . " = '$email'";
        if ($this->db->table($this->user_table)->field($this->field_id)->where($this->field_email . " = '$email'")->getField() > 0) {
            ...
            $sql = "INSERT INTO " . $this->db->pre.$this->user_table . " (" . implode(',', $fields) . ")" . " VALUES ('" . implode("', '", $values) . "')";
...
".$this->field_name." <> '".$cfg['post_username']."'";
            ...
            /*$rs = $this->db->table($this->user_table)->field($this->field_id)->where($this->field_email . " ='$cfg[email]' " . " AND " .$this->field_name . " != '$cfg[post_username]'")->getField();*/
            if (!empty($rs) && $rs > 0) {
                    $this->error = ERR_EMAIL_EXISTS;
