...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a href=\"" .U('user/index/cancel_order', array('order_id'=>$row['order_id'])). "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a href=\"" .U('user/index/affirm_received', array('order_id'=>$row['order_id'])). "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
            }
            ...
            else
            {
                ...
                {
                    @$row['handler'] = '<a href="'. U('user/index/order_detail', array('order_id'=>$row['order_id'])) .'">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] = '<a href="'. U('user/index/order_detail', array('order_id'=>$row['order_id'])) .'">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
...
{
    include_once(ROOT_PATH . 'source/helpers/lib_order.php');
    ...
    if (!empty($order['invoice_no']))
    {
         ...
         $plugin = ROOT_PATH.'source/modules/shipping/'. $shipping_code. '.php';
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            /* 调用相应的支付方式文件 */
            include_once(ROOT_PATH . 'source/modules/payment/' . $payment_info['pay_code'] . '.php');
...
function get_user_bouns_list($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['status'] = '<a href="'. U('user/index/order_detail', array('order_id'=>$row['order_id'])) .'" >' .$GLOBALS['_LANG']['had_use']. '</a>';
        }
