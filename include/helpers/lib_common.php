...
function &init_users()
{
    ...
    $integrate_code = $GLOBALS['_CFG']['integrate_code'] == 'ecshop'?'passport':$GLOBALS['_CFG']['integrate_code'];
    include_once(ROOT_PATH . 'include/modules/integrates/' . $integrate_code . '.php');
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    include_once(ROOT_PATH . ADMIN_PATH . '/Common/lib_template.php');
...
{
    /* 包含加密解密函数所在文件 */
    include_once(ROOT_PATH . 'source/helpers/lib_code.php');
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                ...
                {
                    $uri = U('category/index/index', array('id'=>$cid));
                    if (!empty($bid))
                    {
                        $uri .= '&amp;brand=' . $bid;
                    }
        ...
        case 'goods':
            ...
            {
                $uri = $rewrite ? 'goods-' . $gid : U('goods/index/index', array('id'=>$gid));
            }
        ...
        case 'brand':
            ...
            else
            {
                ...
                {
                    $uri = U('brand/index/index', array('id'=>$bid));
                    if (!empty($cid))
                    {
                        $uri .= '&amp;cat=' . $cid;
                    }
        ...
        case 'group_buy':
            ...
            {
                $uri = $rewrite ? 'group_buy-' . $gbid : U('group_buy/index/view', array('id'=>$gbid));
            }
        ...
        case 'auction':
            ...
            {
                $uri = $rewrite ? 'auction-' . $auid : U('auction/index/view', array('id'=>$auid));
            }
        ...
        case 'exchange_goods':
            ...
            {
                $uri = $rewrite ? 'exchange-id' . $gid : U('exchange/index/view', array('id'=>$gid));
            }
...
function user_uc_call($func, $params = null)
{
    if (isset($GLOBALS['_CFG']['integrate_code']) && $GLOBALS['_CFG']['integrate_code'] == 'ucenter')
    {
        ...
        if (!function_exists($func))
        {
            include_once(ROOT_PATH . 'source/helpers/lib_uc.php');
        }
