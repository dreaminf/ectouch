...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = U('user/index/forget', array('uid'=>$uid, 'code'=>$code), true, true);
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $reset_email = U('user/index/validate_email', array('hash'=>$hash), true, true);
