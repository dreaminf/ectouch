...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            {
                $rows['handle'] = '<a href="'.U('user/index/pay', array('id'=>$rows['id'], 'pid'=>$pid)).'">'.$GLOBALS['_LANG']['pay'].'</a>';
            }
...
function color_tag(&$tags)
{
    ...
    foreach ($tags AS $key => $val)
    {
        ...
        {
            $tags[$key]['url'] = U('search/index/index', array('keywords'=>urlencode($val['tag_words'])));
        }
    }
    shuffle($tags);
}
