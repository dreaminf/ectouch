...
class Category extends IndexController
{
    ...
    {
        $cat_id = I('id', 1);
        if ($cat_id == 0 && CONTROLLER_NAME == 'category') {
            $arg = array(
                'id' => $this->cat_id,
                'brand' => $this->brand,
        ...
        $this->assign('page', $this->parameter['page']);
        $this->assign('size', $this->parameter['size']);
        $this->assign('sort', $this->parameter['sort']);
        $this->assign('order', $this->parameter['order']);
        $this->assign('id', $this->parameter['cat_id']);
        //获取分类信息
        $children = get_children($cat_id);
        $cat_info = get_categories_tree($cat_id);
        foreach ($cat_info as $key => $val) {
            $info[$key]['name'] = $val['name'];
            $info[$key]['url'] = $val['url'];
        }
        $brand = I('request.brand');
        $max = I('request.price_max');
    ...
    public function products()
    {
        ...
        sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION ['user_rank'] . '-' . C('lang') . '-' . $brand . '-' . $price_max . '-' . $price_min . '-' . $filter_attr_str));

        if (!$this->is_cached('category.dwt', $cache_id)) {
            /* 如果页面没有被缓存则重新获取页面的内容 */
            ...
            $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num " . "FROM " . $GLOBALS ['ecs']->table('brand') . "AS b, " . $GLOBALS ['ecs']->table('goods') . " AS g LEFT JOIN " . $GLOBALS ['ecs']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " . "WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array(
                    $cat_id
            ...
            if ($cat ['filter_attr'] > 0) {
                ...
                foreach ($cat_filter_attr as $key => $value) {
                    ...
                    if ($temp_name = $this->load->db->getOne($sql)) {
                        ...
                        $attr_list = $this->loca->db->getAll($sql);
    ...
    private function init()
    {
        ...
        $this->parameter['cat_id'] = 0;
        $this->parameter['children'] = '';
        $this->parameter['brand'] = 0;
        $this->parameter['type'] = '';
        $this->parameter['price_min'] = 0;
        $this->parameter['price_max'] = 0;
        $this->parameter['size'] = 10;
        $this->parameter['page'] = 1;
        $this->parameter['sort'] = 'last_update';
        $this->parameter['order'] = 'ASC';
        $this->parameter['keywords'] = '';
        $this->parameter['filter_attr_str'] = 0;
        ...
        $default_sort_order_type = C('sort_order_type') == '0' ? 'sales_volume' : (C('sort_order_type') == '1' ? 'sales_volume' : 'last_update');
        $sort = I('sort');
        ...
        $this->parameter['display'] = $display;
    ...
    private function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
    {
        ...
        /* 获得商品列表 */
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " . 'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' . 'FROM {pre}goods AS g ' . 'LEFT JOIN {pre}member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE $where ";
        $res = $this->load->db->selectLimit($sql, $size, ($this->parameter['page'] - 1) * $size);
        $arr = array();
        $row = $this->load->db->fetchRow($res);
