    ...
    public function get_wall_msg(){
        if(IS_AJAX && IS_GET){
            ...
            if((!empty($start) || $start === 0) && $num){
                ...
                $list = $Eccache->get($cache_key);
                if(!$list){
                    $sql = "SELECT m.content, m.addtime, u.nickname, u.headimg, u.id FROM ".$this->model->pre."wechat_wall_msg m LEFT JOIN ".$this->model->pre."wechat_wall_user u ON m.user_id = u.id WHERE u.wall_id = '$wall_id' ORDER BY addtime ASC LIMIT ".$start.", ".$num;
                    $data = $this->model->query($sql);
                    $Eccache->set($cache_key, $data, 10);
                    $list = $Eccache->get($cache_key);
                }
