...
class IndexOldController extends CommonController {
    ...
    public function ajax_goods() {
        if (IS_AJAX) {
            ...
            $start = $_POST['last'];
            $limit = $_POST['amount'];
            $goods_list = model('Index')->goods_list($type, 0, $limit, $start);
            $list = array();
            // 热卖商品
            if ($goods_list) {
                foreach ($goods_list as $key => $value) {
                    ...
                    $list [] = array(
                        'single_item' => ECTouch::view()->fetch('library/asynclist_index_old.lbi')
                    );
                }
            }
