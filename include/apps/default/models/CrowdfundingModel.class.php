...
class CrowdfundingModel extends CommonModel {
    ...
    function crowd_goods_info($goods_id = 0) {
		$now = time();
        $sql = 'SELECT goods_id, cat_id, goods_name, goods_img, sum_price, start_time,shiping_time,end_time,gallery_img,status ' . 'FROM ' . $this->pre . 'crowd_goods ' . "WHERE  goods_id = '$goods_id' ";
        $row = $this->row($sql);
        if ($row !== false) {
    ...
    function crowd_buy_price($goods_id = 0) {
        $sql = "SELECT sum(goods_amount) as price FROM " . $this->pre . "crowd_order_info  WHERE goods_id = '" . $goods_id . "' AND extension_code = 'crowd_buy' and pay_status = '" . PS_PAYED . "' ";
        $res = $this->row($sql);
        return $res['price'];
    }
