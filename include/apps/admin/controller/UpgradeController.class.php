...
class UpgradeController extends AdminController
{
    ...
    public function index()
    {
        ...
        $pathlist_str = Http::doGet($upgrade_path_base);
            ...
            foreach ($pathlist as $k => $v) {
                ...
                if (! empty($file_list)) {
                    foreach ($file_list as $fk => $fv) {
                        if (in_array(strtolower(substr($fv, - 3, 3)), array(
                            'php',
                            'sql'
                        ))) {
                ...
                if (file_exists($configpath)) {
                    ...
                    $version_arr = array(
                        'pc_version' => $config_arr['to_version'],
                        'pc_release' => $config_arr['to_release']
                    );
                    // 版本文件地址
            ...
            if ($file != '.' && $file != '..') { // 排除"."和"."
                ...
                if (is_dir($filefrom)) { // 如果是子目录，则进行递归操作
                    ...
                    if (! empty($cover)) {
                        ...
                        if (fileext($fileto) == 'html' && file_exists($fileto)) {} else {
                            if (! copy($filefrom, $fileto)) {
                                $this->copyfailnum ++;
                                echo L('copy') . $filefrom . L('to') . $fileto . L('failed') . "<br />";
                            }
