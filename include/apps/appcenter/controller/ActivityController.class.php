...
class ActivityController extends CommonController {
    ...
    public function wall_prize(){
        ...
        //中奖的用户
        $sql = "SELECT u.nickname, u.headimg FROM ".$this->model->pre."wechat_wall_user u LEFT JOIN ".$this->model->pre."wechat_prize p ON u.openid = p.openid WHERE u.wall_id = '$wall_id' AND u.status = 1 AND u.openid in (SELECT openid FROM ".$this->model->pre."wechat_prize WHERE wall_id = '$wall_id') ORDER BY u.addtime DESC";
        $list = $this->model->query($sql);
    ...
    public function start_draw(){
        if(IS_AJAX){
            ...
            $sql = "SELECT u.nickname, u.headimg FROM ".$this->model->pre."wechat_wall_user u LEFT JOIN ".$this->model->pre."wechat_prize p ON u.openid = p.openid WHERE u.wall_id = '$wall_id' AND u.status = 1 AND u.openid not in (SELECT openid FROM ".$this->model->pre."wechat_prize WHERE wall_id = '$wall_id') ORDER BY u.addtime DESC";
            $list = $this->model->query($sql);
