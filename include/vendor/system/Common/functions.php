...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if(strtolower($var[C('VAR_CONTROLLER')]) == strtolower(C('DEFAULT_CONTROLLER'))){
        unset($var[C('VAR_CONTROLLER')]);
    }
    if(strtolower($var[C('VAR_ACTION')]) == strtolower(C('DEFAULT_ACTION'))){
        unset($var[C('VAR_ACTION')]);
    }

    if (0 == C('URL_MODEL')) {
        // 普通模式URL转换
        $url = __APP__ . '?' . C('VAR_MODULE') . "={$module}";
        $url .= !empty($var) ? '&' . http_build_query(array_reverse($var)) : '';
        if ($urlCase) {
            $url = strtolower($url);
        }
        if (!empty($vars)) {
