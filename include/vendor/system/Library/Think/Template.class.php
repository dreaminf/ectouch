    ...
    public function loadTemplate($templateFile, $prefix = '')
    {
        ...
        if (C('LAYOUT_ON')) {
            if (false !== strpos($tmplContent, '{__NOLAYOUT__}')) {
                ...
                $layoutFile = empty($layoutPath) ? THEME_PATH . C('LAYOUT_NAME') . $this->config['template_suffix'] : $layoutPath . C('LAYOUT_NAME') . $this->config['template_suffix'];
                // 检查布局文件
    ...
    protected function parseLayout($content)
    {
        ...
        if ($find) {
            ...
            if (!C('LAYOUT_ON') || C('LAYOUT_NAME') != $array['name']) {
                // 读取布局模板
                $layoutPath = C('LAYOUT_PATH');
                $layoutFile = empty($layoutPath) ? THEME_PATH . $array['name'] . $this->config['template_suffix'] : $layoutPath . $array['name'] . $this->config['template_suffix'];
